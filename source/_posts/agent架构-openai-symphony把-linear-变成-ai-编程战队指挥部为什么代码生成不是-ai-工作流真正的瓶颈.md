...
img: "https://iili.io/BblpCiv.png"
swiperImg: "https://iili.io/BblpCiv.png"
...
cover: "https://iili.io/BblpCiv.png"
---

<div class="plain-article-asset article-cover-asset"><img src="https://iili.io/BblpCiv.png" alt="【Agent架构 | OpenAI Symphony】把 Linear 变成 AI 编程战队指挥部：为什么代码生成不是 AI 工作流真正的瓶颈"></div>
...
这是安全边界——如果某个 Agent 跑偏了，它的影响范围被锁死在工单目录里。

**状态机驱动** ：工单在 Linear 里的状态直接控制 Agent 的行为流——Todo → In Progress → Human Review → Done。

人类只在 Human Review 节点介入审查，其他状态全由编排器自动流转。
...
![正文图解 2](https://iili.io/Bb01lC7.png)
> 正文图解 2

**Policy 层** ：对应仓库根目录的 `WORKFLOW.md` 文件，里面用 YAML front matter 写配置、用 Markdown 写 prompt 模板，跟着代码一起 git 版本化。

团队规则的演进有了完整的 audit trail，修改一条规则就是开一个 PR。

**Configuration 层** ：负责类型化解析、默认值合并、`$LINEAR_API_KEY` 之类的环境变量替换。这一层把人类的策略意图转成机器可执行的结构化参数。**Coordination 层** ：整个系统的大脑，唯一有权修改调度状态的组件。
...
未来可以扩展 Jira、GitHub Issues 或其他任务追踪系统。
...
重试机制用指数退避——第一次失败等 10 秒，第二次 20 秒，第三次 40 秒，最长不超过 5 分钟。
...
Workspace Manager 为每个任务创建独立文件目录，Agent 只能在自己的目录里操作。这是第一道安全防线。
...
OpenAI 自己说了：OAuth scope 必须做最小权限，能只读就只读，能不加 write 就别加。
...
![正文图解 3](https://iili.io/Bb0183g.png)
> 正文图解 3
