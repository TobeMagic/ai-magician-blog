...
## 二、Checkpoint 持久化：状态机能跨进程恢复，才是生产级 Agent

### 2.1 为什么需要 Checkpoint：一次线上事故说清楚
...
### 2.3 Checkpoint 与 StateGraph 的配合机制
...
## 三、Human-in-the-Loop：把人工审批装进 Agent Loop 的接口设计

### 3.1 什么是 Human-in-the-Loop，以及它为什么是 Harness 层的设计重点
...
### 3.3 安全规则配置：Claude Managed Agents 的实现思路
...
## 四、Tool Calling 编排：Harness 层如何暴露统一的工具调用接口

### 4.1 工具数量爆炸的问题：90.4% 的正确率消失在哪里
...
## 五、AgentRuntime 生命周期：Harness 层管理的四个阶段与接口

### 5.1 四个阶段：初始化 → 调度 → 监控 → 终止与恢复
...
![](https://iili.io/Bii8gls.png)
> 四层接口，我只用过 invoke 和 get_output

## 六、错误隔离与上下文注入：Harness 层两个被低估的面试深水区

### 6.1 错误隔离：为什么 80% 的生产 Agent 事故不是模型的问题
...
## 七、项目落地：如何在面试中把 Harness 设计讲成真实项目

### 7.1 有 Harness 设计痕迹的 AI 项目才能通过追问深度检验
...
### 7.2 项目表达框架：问题定义 → 技术选型 → 实现细节 → 效果量化
...
### 7.4 没有真实项目怎么办：补充 Harness 设计的技术表达
