...
这句话听着像废话，但真正踩过这个坑的人会懂：MCP 定义了「工具长什么样」「调用怎么发出去」「结果怎么返回」，唯独没定义「调用失败了怎么办」。这把可靠性设计的自由度留给了实现者，也把踩坑的概率留给了后来人。
...
这句话听着像废话，但真正踩过这个坑的人会懂：MCP 定义了「工具长什么样」「调用怎么发出去」「结果怎么回来」，但它不管「网络抖了怎么办」。
...
这句话听着像废话，但真正踩过这个坑的人会懂：MCP 定义了「工具长什么样」「调用怎么发出去」「结果怎么回来」，但把超时策略、重试机制、错误分类这些可靠性保障全留给了编排层。
...
这句话听着像废话，但真正踩过这个坑的人会懂：MCP 定义了「工具长什么样」「调用怎么发出去」「结果怎么拿回来」，但它不定义「网络抖了怎么办」「超时了要不要重试」「重试几次间隔多久」。
...
超时控制，这个不用多说，但要注意超时不是越大越好——对下游响应时间做P99统计，超出两倍P99就该考虑降级而不是死等；重试策略，指数退避是基本款，但要配置最大重试次数和熔断阈值，防止雪崩；依赖隔离，关键工具单独配置连接池，别让库存接口拖垮用户中心；健康检查，定期ping下游服务，状态不好时提前标记而不是等超时；降级兜底，接口不可用时给默认返回值或者跳过该步骤，保证主流程不断。
...
![程序员 reaction：ANINCIDENTHASBEENDECLARED](https://iili.io/CxfPjZg.png)
> 背后的逻辑值得细品
